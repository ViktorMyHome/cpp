...
// Binary Heap - complete binary tree where any node <= All its children
// Root has the minimum value in a tree
// in Max heap where any node >= all its children
// top() refers to the min value in the heap
// pop() will remove the smallest value from a heap  
// node i, its children 2*i+1, 2*i+2
// next available node in the tree is O(1)
